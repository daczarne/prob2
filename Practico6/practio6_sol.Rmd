...
donde en la primer desiguladad utilizamos el hecho de que las variables están uniformemente acotadas (por hipótesis), mientras que en la segunda desigualdad utilizamos la desigualdad de Markov.
...
*Dem*: si probamos que $\frac{ 1 }{ B_n^{1 + \frac{\delta}{2}} } \sum\limits_{j = 1}^{n} \E \big( |X_{nj}|^{2 + \delta} \big) \overset{n}{\rightarrow} 0$ implíca que $\frac{ 1 }{ B_n } \sum\limits_{j = 1}^{n} \E \big( |X_{nj}|^2 \mathbb{I}_{ \left\{ |X_{nj}| \geq \varepsilon \sqrt{B_n} \right\} } \big) \overset{n}{\rightarrow} 0$, es decir, que la condición **LY** implíca la condición **L**, entonces queda demostrado que $\frac{1}{\sqrt{B_n}} \sum\limits_{j = 1}^{n} X_{nj} \overset{d}{\rightarrow} Z \sim \text{N}(0, 1)$, dado que la condición **L** implíca la condición $\mathbf{\Lambda}$ de Lindeberg, la cual es condición necesaria y suficiente para la convergencia.
...
Esta desigualdad se cumple dado que en la integral del lado izquierdo estmaos integrando sobre todo $\mathbb{R}$ (es decir, sobre toda la recata real), mientras que en la integral de la derecha estamos integrando sobre una semirrecta de $\mathbb{R}$ dado que $\left\{ x : |x| \geq \varepsilon \sqrt{B_n} \right\} \subset \mathbb{R}$.
...
Supongamos por absurdo que $\sqrt{B_n} \overset{n}{\rightarrow} b < +\infty$. Luego entonces:
$$\frac{1}{\sqrt{B_n}} \sum\limits_{j = 1}^{n} \big( X_j - \E(X_j) \big) = \frac{1}{\sqrt{B_n}} \sum\limits_{j = 1}^{n - 1} \big( X_j - \E(X_j) \big) + \frac{1}{\sqrt{B_n}} \big( X_n - \E(X_n) \big) =$$
$$= \frac{ \sqrt{B_{n - 1}} }{\sqrt{B_n}} \underbrace{ \frac{1}{\sqrt{B_{n - 1}}} \sum\limits_{j = 1}^{n - 1} \big( X_j - \E(X_j) \big) }_{ \overset{d}{\rightarrow} Z \sim \text{N}(0, \, 1) \text{ por hipótesis}} + \underbrace{ \frac{1}{\sqrt{B_n}} \big( X_n - \E(X_n) \big) }_{ \overset{n}{\rightarrow} (c/b) } \overset{d}{\not{\rightarrow}} Z \sim \text{N}(0, \, 1)$$
Lo cual es un absurdo dado que la convergencia era la hipótesis de partida.
...
donde la desigualdad se cumple dado que $X$ es una variable aletoria de recorrido acotado entre $a$ y $b$.
...
donde $0 \leq |X_n(\omega) - \mu| \leq |Y_n(\omega) - \mu|$. Luego multiplicamos ambos lados de la igualdad por $\sqrt{n}$ y reacomodamos términos para obtener:
$$\begin{array}{rcl}
...
Provar que si $\E \left( |X|^n \right) < +\infty$ entonces: $\E \left( |X|^n \right) = n \int\limits_{0}^{+\infty} x^{n - 1} \big( F_X(-x) + 1 - F_X(x) \big) dx$
...
Trabajndo con $I_2$ obtenemos que, si integramos por partes:
$$\begin{array}{rcl}
