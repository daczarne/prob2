...
$$\begin{array}{ l l }
f_{X_n}(x;\theta) = \mathbb{I}_{ \{ \theta < x < \theta + 1 \} } & F_{X_n}(x;\theta) = (x - \theta) \mathbb{I}_{ \{ \theta < x < \theta + 1 \} } \\ \\
f_{X_{1:n}}(x;\theta) = n \, (1 - x + \theta)^{n - 1} \, \mathbb{I}_{ \{ \theta < x < \theta + 1 \} } & F_{X_{1:n}}(x;\theta) = 1 - (\theta + 1 - x)^n \mathbb{I}_{ \{ \theta < x < \theta + 1 \} }
\end{array}$$

$$P ( | X_n - \theta | > \varepsilon) = 1 - P ( | X_n - \theta | < \varepsilon) = 1 - P ( - \varepsilon < X_n - \theta < \varepsilon) = $$
$$ = 1 - P ( \theta < X_n < \theta + \varepsilon) = 1 - \big[ F_{X_{1:n}}( \theta + \varepsilon) - F_{X_{1:n}}( \theta) \big] = 1 - F_{X_{1:n}}( \theta + \varepsilon) + F_{X_{1:n}}( \theta) =$$
$$= 1 - \big[ 1 - (\theta + 1 - \theta - \varepsilon)^n \big] + \big[ 1 - (\theta + 1 - \theta)^n \big] =$$
$$= (1 - \varepsilon)^n + 1 - 1^n = (\underbrace{ 1 - \varepsilon }_{\leq 1})^n \overset{n}{\rightarrow} 0  $$
